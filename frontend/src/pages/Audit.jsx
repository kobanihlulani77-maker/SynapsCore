...
export default function AuditPage({ context }) {
  ...
  const rawActivitySuppressed = (signedInWarehouseScopes || []).length > 0
  const evidenceCount = snapshot.auditLogs.length + snapshot.recentEvents.length
  const scopeMessage = 'Raw tenant Activity and Audit evidence is not available for this warehouse-scoped role.'
  const evidenceMetric = rawActivitySuppressed ? 'Hidden' : evidenceCount
    ...
    <section className="content-grid">
      <Panel wide>
        <div className="panel-header">
          ...
          <span className="panel-badge audit-badge">{rawActivitySuppressed ? 'Scoped' : evidenceCount}</span>
        </div>
        <OperationalGuidance
          stateLabel={pageLoading ? 'Loading evidence' : pageError ? 'Unavailable' : rawActivitySuppressed ? 'Scope-limited' : 'Observed timeline'}
          stateTone={pageError ? 'status-failure' : pageLoading || rawActivitySuppressed ? 'status-partial' : 'status-success'}
          stateDetail={pageLoading ? 'Business events and audit entries are still loading.' : pageError ? 'The activity read is unavailable; do not interpret the visible feed as empty.' : rawActivitySuppressed ? scopeMessage : `${evidenceCount} recent evidence record${evidenceCount === 1 ? '' : 's'} are visible.`}
          attention={rawActivitySuppressed ? 'Raw tenant-wide evidence is intentionally suppressed; warehouse-scoped operational feeds remain available on their own pages.' : systemIncidents.length || pendingReplayCount ? `${systemIncidents.length} runtime incident${systemIncidents.length === 1 ? '' : 's'} and ${pendingReplayCount} replay item${pendingReplayCount === 1 ? '' : 's'} may require a related evidence review.` : 'No incident or replay pressure is currently attached to the visible timeline.'}
          nextAction={rawActivitySuppressed ? 'Use the warehouse-scoped operational pages for permitted activity and evidence.' : selectedAuditTrace ? 'Inspect the selected fact, then follow Runtime or Replay for operational recovery context.' : 'Select an event or audit entry to inspect its timing and trace metadata.'}
        ...
        <div className="summary-grid compact-summary-grid">
          <SummaryCard label="Audit entries" value={rawActivitySuppressed ? 'Hidden' : snapshot.auditLogs.length} accent="blue" />
          <SummaryCard label="Business events" value={rawActivitySuppressed ? 'Hidden' : snapshot.recentEvents.length} accent="teal" />
        ...
        <div className="experience-grid experience-grid-split">
          <article className="stack-card section-card" id="audit-events">
            ...
            <div className="signal-list">
              ...
              )) : <EmptyState>{rawActivitySuppressed ? scopeMessage : 'Business events will stream here as SynapseCore processes operational change.'}</EmptyState>}
            </div>
          </article>
          <article className="stack-card section-card" id="audit-logs">
            ...
            <div className="signal-list">
              ...
              )) : <EmptyState>{rawActivitySuppressed ? scopeMessage : 'Audit traces will appear once protected operational actions are flowing.'}</EmptyState>}
            </div>
          </article>
        </div>
        <div className="experience-grid experience-grid-split">
          <article className="stack-card section-card">
            ...
            ) : <EmptyState>{rawActivitySuppressed ? scopeMessage : 'Select an audit entry or business event to inspect the exact trace lane and timing context.'}</EmptyState>}
          </article>
          <article className="stack-card section-card">
            <div className="stack-title-row"><strong>Traceability posture</strong><span className="scenario-type-tag">{rawActivitySuppressed ? 'Scoped' : evidenceMetric}</span></div>
            <div className="utility-metric-grid">
              <div><span>Audit</span><strong>{rawActivitySuppressed ? 'Hidden' : snapshot.auditLogs.length}</strong></div>
              <div><span>Events</span><strong>{rawActivitySuppressed ? 'Hidden' : snapshot.recentEvents.length}</strong></div>
