...
class RealtimeServiceTest {
    ...
    void broadcastOperationalUpdatesPublishesFocusedRealtimeTopics() {
        ...
        List<IntegrationConnectorResponse> integrationConnectors = List.of(
            new IntegrationConnectorResponse(1L, "STARTER-OPS", "erp_north", IntegrationConnectorType.WEBHOOK_ORDER, "ERP North Webhook", true, IntegrationSyncMode.REALTIME_PUSH, List.of(IntegrationSyncMode.REALTIME_PUSH, IntegrationSyncMode.SCHEDULED_PULL), null, null, null, null, null, null, IntegrationValidationPolicy.STANDARD, IntegrationTransformationPolicy.NORMALIZE_CODES, 1, true, "WH-NORTH", "Starter webhook connector.", "Operations Lead", "Operations Lead", true, "••••2026", IntegrationConnectorHealthStatus.LIVE, "Connector is enabled and processing activity without recent integration failures.", "Webhook order feeds support realtime push and scheduled pull.", Instant.now(), Instant.now(), null, null, 0, 0, 0, null, null, null, null, null, Instant.now(), Instant.now())
        );
    ...
    void broadcastIntegrationUpdatesPublishesFocusedIntegrationTopics() {
        ...
        List<IntegrationConnectorResponse> integrationConnectors = List.of(
            new IntegrationConnectorResponse(1L, "STARTER-OPS", "erp_north", IntegrationConnectorType.WEBHOOK_ORDER, "ERP North Webhook", false, IntegrationSyncMode.REALTIME_PUSH, List.of(IntegrationSyncMode.REALTIME_PUSH, IntegrationSyncMode.SCHEDULED_PULL), null, null, null, null, null, null, IntegrationValidationPolicy.STANDARD, IntegrationTransformationPolicy.NORMALIZE_CODES, 1, true, "WH-NORTH", "Paused for maintenance.", "Operations Lead", "Operations Lead", true, "••••2026", IntegrationConnectorHealthStatus.OFFLINE, "Connector is disabled and cannot ingest live activity.", "Webhook order feeds support realtime push and scheduled pull.", null, null, null, null, 0, 0, 0, null, null, null, null, null, Instant.now(), Instant.now())
        );
        List<IntegrationImportRunResponse> integrationImportRuns = List.of(
            new IntegrationImportRunResponse(4L, "erp_batch", IntegrationConnectorType.CSV_ORDER_IMPORT, "orders.csv", 3, 1, 1, IntegrationImportStatus.PARTIAL_SUCCESS, "Processed CSV import with 1 imported order and 1 failure.", Instant.now())
        );
