...
- `REVIEW_OWNER` acts only on assigned review-stage work and may execute an eligible scenario;
- `FINAL_APPROVER` acts only on assigned final-stage work and may execute an eligible scenario;
...
| --- | --- | --- | --- | --- |
| `TENANT_ADMIN` | Common workspace, Users, Company Settings; tenant-wide or explicitly approved scope | Verify roster, roles, scopes, settings, warehouse metadata, support ownership; perform approved access/config changes | Enter Platform Admin; use full integration/replay resources; approve/execute scenarios unless separately assigned; browse another tenant | Wrong identity/scope, unexplained account, configuration drift, platform route/API access, foreign data |
| `INTEGRATION_ADMIN` | Common workspace, Integrations, Replay Queue, connector policy controls | Confirm connector state/policy; inspect imports/failures; authorize and perform safe replay; apply approved connector changes | Manage tenant users; make governance decisions; execute scenarios by this role; replay with unresolved source/duplicate risk | Connector/source mismatch, secret concern, missing failure evidence, replay uncertainty, wrong warehouse |
| `INTEGRATION_OPERATOR` | Common workspace, read-only connector/import views, Replay Queue within scope | Observe integration state; investigate failures; replay eligible corrected records once when authorized | Mutate connector policy; manage users; perform governance actions; bypass warehouse scope | Record not visible, eligibility unclear, dead-lettered item, duplicate risk, disabled-webhook limitation |
| `REVIEW_OWNER` | Common workspace and Approvals for assigned review-stage work | Review evidence; approve/reject assigned review stage; execute only eligible, authorized scenarios in scope | Substitute for final approver/escalation owner; manage connectors/users/platform; act outside assignment/scope | Wrong stage/actor, stale scenario, insufficient evidence, execution effect unclear |
| `FINAL_APPROVER` | Common workspace and Approvals for assigned final-stage work | Make assigned final decision with required evidence; execute eligible authorized scenarios in scope | Perform review-stage substitution; acknowledge escalation by this role; manage connector/users/platform | Separation-of-duty issue, requester/reviewer conflict, wrong assignment, stale or unsafe execution |
