...
async function loginViaUi(page, credentials) {
  ...
  await expect(page.getByRole('heading', { name: 'Open your company command center' })).toBeVisible()
  await page.getByLabel('Tenant workspace').selectOption(credentials.tenantCode)
  await page.getByLabel('Username').fill(credentials.username)
  await page.getByLabel('Password').fill(credentials.password)
  await page.locator('.public-signin-card').getByRole('button', { name: 'Sign In' }).click()
...
test('auth flow and the full authenticated page system render cleanly in a browser', async ({ page }) => {
  ...
  await page.getByLabel('Tenant workspace').selectOption(users.operationsLead.tenantCode)
