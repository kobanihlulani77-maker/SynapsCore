...
backend_ddl_auto="$(get_env_value "$BACKEND_FILE" "SPRING_JPA_HIBERNATE_DDL_AUTO")"
...
echo "  Hibernate mode   : $backend_ddl_auto"
echo "  Flyway enabled   : ${backend_flyway_enabled:-true}"
echo
...
echo
echo "Final signoff lane"
echo "  Config gate      : BACKEND_ENV_FILE=$BACKEND_ENV_FILE FRONTEND_ENV_FILE=$FRONTEND_ENV_FILE bash scripts/check-prod-config.sh"
echo "  Hosted prep      : powershell -ExecutionPolicy Bypass -File scripts/prepare-hosted-proof.ps1"
echo "  Hosted proof     : cd frontend && npm.cmd run test:e2e:prod"
echo
...
echo
echo "Hosted proof is the primary final signoff lane."
echo "Localhost verification remains useful for self-host smoke only."
echo
