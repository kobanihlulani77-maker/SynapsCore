...
- Backend scenario assignment remains authoritative: cross-warehouse Review Owner assignment is rejected, while a valid warehouse-eligible assignment is accepted. The governance invariant now also rejects requester/self-review at save, approval, and rejection time; default Review Owner resolution excludes the requester. Existing backend authority tests remain in place, with a regression test covering both new rejection points.
