...
set_env_value "$BACKEND_TARGET" "SPRING_FLYWAY_ENABLED" "true"
set_env_value "$BACKEND_TARGET" "SPRING_JPA_HIBERNATE_DDL_AUTO" "validate"
...
echo
echo "Prepared disposable free-test deployment env files:"
echo "  $BACKEND_TARGET"
...
echo "  $DB_PASSWORD"
echo
echo "These env files are for disposable free-test/self-host verification only."
echo "They are not the primary hosted-proof signoff lane."
echo
echo "Next steps:"
...
echo "4. Primary final signoff still runs through prepare-hosted-proof.ps1 plus frontend npm.cmd run test:e2e:prod"
